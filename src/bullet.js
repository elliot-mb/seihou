export default class Bullet{ //exports the class for use in game.js
    ...
    update(deltaTime, endless){
        ...
        try{
            ...
            this.deltaX = (this.polarity * this.scaledSpeed) * Math.cos(this.theta);    //(Math.sqrt(Math.pow(this.scaledSpeed, 2) - Math.pow(gradient, 2))) ||| ((Math.abs(Math.pow(this.scaledSpeed, 2) - Math.pow(this.gradient, 2))))
            ...
            if (Math.abs(this.gradient) < this.verticalLim){ //this if/else statement handles exceptions where the bullet is being shot directly up or down, as the gradient is near infinite and cannot be worked with
                ...
                this.position.y += (this.scaledSpeed * this.polarity * deltaTime);
            }
            ...
            this.scaledSpeed += (this.deltaSpeed/1000) * deltaTime;
